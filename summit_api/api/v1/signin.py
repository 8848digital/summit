import frappe
from summit_api.utils import check_user_exists,success_response,error_response, resync_cart
...
def signin(kwargs):
	try:
		...
		if kwargs.get('with_otp'):
			# Validate OTP
			from summit_api.api.v1.otp import verify_otp
...
def signin_as_guest(kwargs):
	try:
		...
		"""
		from summit_api.api.v1.registration import create_user, create_customer, create_address
...
def login_via_google(kwargs):
	if check_user_exists(kwargs.get('usr', kwargs.get("email"))):
		...
		from summit_api.api.v1.registration import customer_signup
