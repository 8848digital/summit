...
def get_item_field_values(currency,item, customer_id, url_type,field_names):
    computed_fields = {
		...
		'equivalent': lambda: {'equivalent': bool(int(item.get('equivalent')))},
		'alternate': lambda: {'alternate': bool(int(item.get('alternate')))},
		'mandatory': lambda: {'mandatory': bool(int(item.get('mandatory')))},
		'suggested': lambda: {'suggested': bool(int(item.get('suggested')))},
