...
def get(kwargs):
	try:
		...
		category_list = [
			{"values": get_sub_cat(cat, allowed_categories=categories), **cat}
...
def breadcrums(kwargs):
	try:
		listing_map = {"listing": "product-category", "brand": "brand", "catalog": "catalog"}
		...
		if not brand:
			for cat in parent_categories:
				...
				res.append({"name": cat.label or cat.name, "link": url})
				last_cat = cat.slug
			if product:
				res.append(
					{
