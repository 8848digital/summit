...
def create_warranty_claim(kwargs):
	try:
		if frappe.request.data:
			...
			exs_cust_wc = frappe.get_list(
				"Customer Warranty Claim", filters={"customer": wc_doc.customer}, fields=["*"]
			)
...
def get_warranty_claim(kwargs):
	try:
		...
		warranty_claim = frappe.get_list(
			"Warranty Claim", filters={"serial_no": serial_no}, fields=filed_names
		)
...
def get_sr_no_list(kwargs):
	try:
		...
		sr_no = frappe.get_list(
			"Serial No", filters={"item_code": item_code}, fields=filed_names
		)
...
def get_sr_no_details(kwargs):
	try:
		...
		serial_no = kwargs.get("serial_no")
		sr_no = frappe.get_list(
			"Serial No", filters={"serial_no": serial_no}, fields=filed_names
		)
		return success_response(data=sr_no)
	except Exception as e:
		frappe.logger("warranty").exception(e)
...
def get_cust_wc_details(kwargs):
	try:
		...
		cust_wc_details = frappe.get_list(
			"Customer Warranty Claim", filters={"name": customer}
		)
		result = {"warranty_details": get_details(cust_wc_details[0].name)}
...
def get_details(doc):
	try:
		details = frappe.get_all(
			...
			filters={"parent": doc},
			fields=["warranty_claim", "serial_no", "status", "warranty_amc_status"],
		)
		return details
	except Exception as e:
		frappe.logger("warranty").exception(e)
