...
def get_dealer(kwargs):
	try:
		...
		if frappe.session.user != "Guest":
			dealer_list = frappe.db.get_list("Customer", filters=filters, fields=["name"])
		else:
			dealer_list = frappe.db.get_list(
				"Customer", filters=filters, fields=["name"], ignore_permissions=True
			)
		result = []
		...
		if company_name:
			if result:
				result = list(filter(lambda d: d["Trading / Company Name"] == company_name, result))
