...
def get(kwargs):
	try:
		...
		if address := kwargs.get("address_id"):
			...
			address_all_doc = frappe.get_all(
				"Dynamic Link", {"link_doctype": "Customer", "link_name": user_id}, "*"
			)
...
def put(kwargs):
	try:
		...
		email = (
			kwargs.get("email") if "random" in frappe.session.user else frappe.session.user
		)
		user_id = kwargs.get("user_id")
		if not kwargs.get("user_id"):
			user_id = frappe.get_value("Customer", {"email": email})
		if not user_id:
			user_id = update_customer(
				data={
			...
			if quot_name := frappe.db.exists(
				"Quotation",
				{"status": "Draft", "owner": frappe.session.user, "party_name": ["is", "null"]},
			):
...
def get_details(customer, address_obj):
	return {
		...
		"set_as_default": bool(address_obj.is_primary_address)
		if address_obj.get("address_type") == "Billing"
		else bool(address_obj.is_shipping_address),
...
def add_address(kwargs, user_id):
	if kwargs.get("address_id"):
		return update_address(kwargs, kwargs.get("address_id"))
	doc_dict = {
		"doctype": "Address",
	...
	doc.append("links", {"link_doctype": "Customer", "link_name": user_id})
...
def get_address(customer, customer_address, shipping_address):
	...
	res = []
	res.append(
		get_address_detail_json("Billing Address", customer, customer_address_doc)
	) if customer_address_doc else ""
	res.append(
		get_address_detail_json("Shipping Address", customer, shipping_address_doc)
	) if shipping_address_doc else ""
	return res
...
def get_address_detail_json(type, customer, address_doc):
	return {
		"name": type,
		"values": [get_details(customer, address_doc)] if address_doc else [],
	}


def update_customer(customer=None, data=None):
	if data is None:
		data = {}
	if customer:
