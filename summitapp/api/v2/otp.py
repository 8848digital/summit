...
def send_pinnacle_sms(kwargs):
	try:
		...
		print(
			"111",
			pinnacle_settings.url,
			pinnacle_settings.apikey,
			pinnacle_settings.sender,
			pinnacle_settings.messagetype,
			pinnacle_settings.dlttempid,
			pinnacle_settings.contenttype,
		)
		phone = kwargs.get("phone")
		phone_number = f"+{phone}"
		otp_length = 6
		...
		response = requests.request(
			"POST", pinnacle_settings.url, headers=headers, data=payload
		)
