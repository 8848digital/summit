...
from summitapp.api.v2.utilities.resync_cart import resync_cart
from summitapp.api.v2.utilities.user import check_user_exists
from summitapp.api.v2.utilities.utils import error_response, success_response


def signin(kwargs):
	try:
		if (kwargs.get("usr", kwargs.get("email"))) == frappe.session.user:
			return success_response(data="Already Logged In")
		...
		if kwargs.get("with_otp"):
			...
			if (
				verify_otp({"email": kwargs.get("usr"), "otp": kwargs.get("pwd")}).get("msg")
				!= "success"
			):
...
def signin_as_guest(kwargs):
	try:
		...
		"""
		from summitapp.api.v2.registration import (create_address, create_customer,
		                                           create_user)
		...
		return success_response(
			data={"address_id": address_doc.name, "customer_id": customer_doc.name}
		)
...
def get_user_profile(kwargs):
	...
	return success_response(data={"is_superadmin": is_superadmin, "is_dealer": is_dealer})
