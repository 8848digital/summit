...
from summitapp.api.v2.product.product_list import get_item
...
def get_wishlist_items(kwargs):
	...
	try:
		if frappe.session.user == "Guest":
			return error_response("Please Login first.")
		wishlist_items = frappe.db.get_list(
			"Wishlist Item",
		...
		response = {"msg": "success", "wishlist_count": len(wishlist_items), "data": result}
		return response
