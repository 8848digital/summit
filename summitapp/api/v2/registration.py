...
from summitapp.api.v2.utilities.email import send_mail
...
def delete_documents(kwargs, id):
	...
	frappe.local.login_manager.login_as("Administrator")
	frappe.delete_doc(
		"User", kwargs.get("email"), ignore_permissions=True, ignore_missing=1
	)
...
def create_address(kwargs, id):
	...
	address_doc.append("links", {"link_doctype": "Customer", "link_name": id})
	...
	user_doc = frappe.get_doc(
		{
			...
			"mobile_no": kwargs.get("contact_no")
			or kwargs.get("contact")
			or kwargs.get("phone"),
	...
	customer_doc = frappe.get_doc(
		{
			...
			or kwargs.get("contact")
			or kwargs.get("phone"),
			"mobile_number": kwargs.get("contact_no")
			or kwargs.get("contact")
			or kwargs.get("phone"),
			"email_id": kwargs.get("usr") or kwargs.get("email"),
			"email": kwargs.get("usr") or kwargs.get("email"),
			"type": "Individual",
			"customer_group": kwargs.get(
				"customer_group",
...
def send_reset_link(kwargs):
	try:
		...
		if not kwargs.get("link"):
			return error_response("Please Send Redirection Link")
