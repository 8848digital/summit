...
from summitapp.api.v2.customer_address import \
    get_details as get_address_details
...
def get_customer_profile(cust_doc):
	try:
		...
		if cust_doc.get("assigned_coupon_code"):
			allowed_coupons = frappe.get_list(
				"Available Coupons",
				{"parent": cust_doc.name},
				pluck="coupon_name",
				ignore_permissions=1,
			)
			coupon_codes = frappe.get_list(
				"Coupon Code",
				{"coupon_name": ["in", allowed_coupons]},
				pluck="coupon_code",
				ignore_permissions=1,
			)
		contact = (
...
def get_credit_limit(cust_doc, company=None):
	...
	credits = (
		frappe.db.get_value(
			"Customer Credit Limit", {"parent": cust_doc.name}, "credit_limit"
		)
		or 0
	)
	return credits
...
def get_dealer_profile(dealer_doc):
	try:
		...
		dealer_details = {
			...
			"billing_address": cust_bill_address.get("full_address")
			if cust_bill_address
			else "",
			"delivery_address": cust_ship_address.get("full_address")
			if cust_ship_address
			else "",
...
def get_store_credit(user):
	try:
		user_list = frappe.db.get_list(
			"Store Credit Assigned", {"user": user}, ["user as user_name"], group_by="user"
		)
		for user in user_list:
			user["store_credit_assigned"] = sum(
				i.credit_amount
				for i in amount_list("Store Credit Assigned", user.user_name, "credit_amount")
			)
			user["store_credit_used"] = sum(
				i.debit_amount
				for i in amount_list("Store Credit Used", user.user_name, "debit_amount")
			)
			user["store_credit_balance"] = (
				user["store_credit_assigned"] - user["store_credit_used"]
			)
