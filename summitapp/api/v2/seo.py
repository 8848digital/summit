...
def brand_product_urls():
	...
	items = frappe.get_all(
		"Item",
		{"slug": ["is", "set"], "published_in_website": 1, "brand": ["is", "set"]},
		["slug", "brand"],
	)
	result = [
		generate_urls(["brand-product", BRANDS.get(item.brand), item.slug]) for item in items
	]
	return result
...
def catalog_product_urls():
	...
	result = [
		generate_urls(["catalog-product", CATALOGS.get(item.parent), item.item_slug])
		for item in items
	]
	return result
...
def product_urls():
	...
	sub_cat = frappe.get_all(
		"Sub Category", {"slug": ["is", "set"]}, ["slug", "name", "category"]
	)
	SUB_CATEGORIES = {
		row.name: {"slug": row.slug, "parent": row.category} for row in sub_cat
	}
	sub_subcat = frappe.get_all(
		"Sub Category", {"slug": ["is", "set"]}, ["slug", "name", "sub_category_name"]
	)
	result = []
...
def product_page_urls():
	...
	items = frappe.get_all(
		"Item",
		{
			...
			"category": ["is", "set"],
			"sub_category": ["is", "set"],
			"level_three_category_name": ["is", "set"],
		},
		["slug", "category", "sub_category", "level_three_category_name as sub_subcat"],
	)
	result = []
	for item in items:
		lvl4 = item.slug
