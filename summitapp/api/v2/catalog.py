...
from summitapp.api.v2.product.product_list import get_list as get_item_details
from summitapp.api.v2.utilities.utils import error_response, success_response


def get(kwargs):
	try:
		catalog_list = frappe.get_list(
			"Catalog", {}, "*", ignore_permissions=1, order_by="sequence"
		)
		result = [
			{
				"id": catalog.name,
				...
				"product_counts": len(get_item_list(catalog.name)),
				"url": f"catalog/{catalog.slug}",
			}
...
def get_items(kwargs):
	try:
		...
		for item in item_list:
			...
			results.extend(
				result
			)  # Use extend() instead of append() to add the dictionaries directly
...
def get_item_list(catalog):
	...
	items = frappe.get_list(
		"Item Child",
		{"parent": catalog, "parenttype": "Catalog"},
		pluck="item",
...
def delete_item(catalog_name, item):
	...
	frappe.db.delete("Item Child", {"parent": cat_doc.name, "item": item_doc.name})
