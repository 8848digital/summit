...
doc_events = {
	...
	"Contact": {"validate": "summitapp.overrides.contact.validate"},
	"Customer Group": {"validate": "summitapp.overrides.customer_group.validate"},
	"Sales Invoice": {
	...
	"*": {"validate": "summitapp.api.v2.utilities.utils.autofill_slug"},
	"Address": {"before_validate": "summitapp.overrides.address.before_validate"},
}
...
jinja = {"methods": ["summitapp.api.v2.stock.check_availability"]}

scheduler_events = {
	"cron": {
		"* * * * *": [
			"summitapp.api.v2.access_token.generate_keys",
		]
	},
}
...
fixtures = [{"dt": "Custom Field", "filters": [["module", "=", "SummitApp"]]}]
