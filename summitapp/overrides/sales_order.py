...
from summitapp.overrides.payment_entry import make_payment_entry
...
def on_submit(self, method=None):
	if self.store_credit_used:
		...
		discount_acc, receivable_acc = frappe.get_cached_value(
			"Company", doc.company, ["default_discount_account", "default_receivable_account"]
		)
		doc.append(
			"accounts",
			{
		...
		doc.submit()
		frappe.db.set_value(
			"Customer", self.customer, "balance_amount", balance - self.store_credit_used
		)
